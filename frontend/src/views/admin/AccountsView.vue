...
const syncPendingListChanges = async () => {
  ...
  usageManualRefreshToken.value += 1
  appStore.showSuccess(t('admin.accounts.listSynced'))
}
...
const handleToggleSchedulable = async (a: Account) => {
  ...
  try {
    ...
    const actualSchedulable = updated?.schedulable ?? nextSchedulable
    updateSchedulableInList([a.id], actualSchedulable)
    enterAutoRefreshSilentWindow()
